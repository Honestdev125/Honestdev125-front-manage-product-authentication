...
const TextItem: React.FC<TextItemProps> = ({ item_id }) => {
  const { jsonData, setJsonData } = useJsonData();

  const handleChangeTextItem = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (targetIndex !== -1) {
      const updatedItem = {
        ...
        itemData: { ...updatedJsonData.itemsData[targetIndex].itemData, text: value }
      };
  ...
  return (
    <div>
      <input
        ...
        className="bg-gray-50 border border-gray-300 text-gray-900 text-xl rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 my-2 outline-none"
        placeholder="文章を入力"
        required
      />
    </div>
